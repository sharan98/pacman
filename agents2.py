...
class Agent(pygame.sprite.Sprite):
    ...
    def set_grid(g):
        Agent.grid = g
        Agent.gridlist = g.gridwallsList
        Agent.maze = g.maze
    ...
    def __init__(self, color, pos, img):
        ...
        self.image = i #pygame.surface.Surface([width, height])
    ...
    def willHitWall(self, newdirection):
        ...
        if future_pos.collidelist([l.rect for l in Agent.gridlist]) == -1:
            return False
        return True
...
class pacman(Agent):
    ...
    def __init__(self, color, pos):
        img = pygame.image.load('pac_down.jfif')
        Agent.__init__(self, color, pos, img)
    ...
    def update(self, key = None):
        ...
        if not self.willHitWall(newdirection):
            ...
            if changed:
                ...
                self.angle = angle
            # img = pygame.image.load(imgfile + '.jfif')
...
class ghost(Agent):
    ...
    def __init__(self, color, pos, direction, id):
        self.id = id
        filename = ''
        if self.id ==BLINKY:
            filename = 'red.gif'
    ...
    def update(self, key = None):
        ...
        astar(Agent.maze, me, pac, p)
        # oldspeed = self.direction * (GHOST_SPEED * ghost.speed_scale)
        olddirection = newdirection = self.direction
        # p = p[::-1]
        p.reverse()
        try:
            next_pos = p[1]
            ...
            if not self.willHitWall(newdirection):
                ...
                self.speed = olddirection * (GHOST_SPEED * ghost.speed_scale) #oldspeed
        except:
            self.speed = olddirection * (GHOST_SPEED * ghost.speed_scale)#STOP * 2
